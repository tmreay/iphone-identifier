...
- **A run that is still standing stays in the trail on every view.** Browsing the
  models mid-run does not stop it (D-25), so the list and the entries reached
  from it hang off the run rather than replacing it, and its crumb is the way
  back that costs nothing.
- **A stage is named only when there provably is one.** A fresh run has none, so
  none is named — including on an entry bookmarked mid-run and reopened after a
  reload, where the hash survives and the answer trail does not. This is the same
  restraint the entry's back label already shows (§4.5, D-25): the trail says a
  fresh identification is above you rather than naming a question that was never
  asked.
