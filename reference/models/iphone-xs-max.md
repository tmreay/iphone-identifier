...
| `body_size_class`    | `large`                           | ✅ verified | S1     | Derived from body height 157.5 mm against the bands in SPEC.md §6.3. The three bands sit in the gaps between the clusters the bodies form, and an adjacent class is added only when a model actually in that class sits within 3 mm — on the current set nothing does, so this is one class (SPEC.md §6.3, D-27). |
